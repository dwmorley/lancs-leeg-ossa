...
def _asd_via_rpy2_inner(
    ...
    with localconverter(pandas2ri.converter):
        ...
        _prog(0.15, "Fitting model (spglm)...")
        ro.r(
            f"""
                model <- spglm(
                    {formulaf},
        ...
        ro.r(
            f"""modelgrid <- mba.surf(cbind(area[,1:2],abs(modelse[,3]-modelse[,2])),no.X={x_res}, no.Y={y_res}, extend=TRUE)$xyz.est"""
        )
        ...
        if target == "H":
            ro.r(
                f"""modelgridX=mba.surf(cbind(area[,1:2],modelse[,1]),no.X={x_res}, no.Y={y_res}, extend=TRUE)$xyz.est"""
            )
            ro.r(
                """x=cbind(expand.grid(modelgrid[[1]],modelgrid[[2]]),c(modelgridX[[3]]),c(modelgrid[[3]]))"""
            )
            ro.r("""x=x[order(x[,3],x[,4],decreasing=T),]""")
